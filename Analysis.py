...
def app():
    ...
    @st.cache_data
    def dataframe():
        df = pd.read_csv('Cleaned_Car_Dheko.csv')  # Read CSV data
        return df
    df = dataframe()  # Call the function to load data

    # Option for the user to select a column to explore
    choice = st.selectbox("**Select an option to Explore their data**", (df.drop('Car_Price', axis=1).columns))

    # Price vs selected feature histogram
    st.markdown(f"## :rainbow[Car Price vs {choice}]")
    hist = px.histogram(df, x=choice, y='Car_Price', width=950, height=500)
    st.plotly_chart(hist)

    # Car brand availability by fuel type
    ...
    # Car model with the highest prices
    ...
    # Car model with the lowest prices
    ...
    # Select car brand and model for further exploration
    ...
    # Car model price vs production year bar chart
    ...
    # Scatter plot for kilometers driven vs car price
    ...
    # Scatter plot for mileage vs car price
    ...
    # Scatter plot for car age vs car price
    ...
    # Location-based analysis with central tendency options
    col, col1 = st.columns([2, 2])
    with col:
    ...
    # Apply central tendency function (mean, median, or mode)
    if select_CT == 'mean':
    ...
    # Pie chart for car prices by model at specific location and brand
